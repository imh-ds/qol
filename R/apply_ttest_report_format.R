...
#'
#' @export
apply_ttest_report_format <- function(
  ...
  header_variable_center_style <- openxlsx::createStyle(
    textDecoration = "bold",
    border = "top-bottom",
    halign = "center"
  )
  ...
  openxlsx::addStyle(wb,
                     sheet = sheet,
                     style = header_variable_center_style,
                     cols = (start_col + 1):(start_col + mod_col - 1),
                     rows = start_row + 1)
  openxlsx::addStyle(wb,
                     sheet = sheet,
                     style = header_variable_p_style,
                     cols = (start_col + mod_col - 4):(start_col + mod_col - 2),
                     rows = start_row + 1)
  ...
  openxlsx::addStyle(wb,
                     ...
                     style = body_style,
                     gridExpand = TRUE,
                     cols = c((start_col + 3):(start_col + mod_col - 1)),
